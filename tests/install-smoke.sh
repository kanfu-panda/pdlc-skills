...
# 真机验证坐实的两条：skill 运行时没有 CLAUDE_PLUGIN_ROOT，同版本脚本只能在 skill 自己的目录下找——
# 所以脚本随 skill 自带在 scripts/ 下（adapters/sync_skills.py 同步）；
# relate set 遇到数组形态的 relations 若直接初始化六类空块，会静默覆盖原有条目。都用断言钉住，防退回。
assert_contains "state-read runs the lint script shipped inside the skill" \
  "\`scripts/pdlc-state-lint.sh\`" "$(cat references/templates/prompts/state-read.md)"
