...
**pdlc-skills** 把「AI 做软件工程」变成一台**可审计、落盘的状态机**。它是一个 [Claude Code plugin](https://docs.anthropic.com/)，提供分阶段的 PDLC（产品开发生命周期）工作流——PRD → 设计 → TDD → 实现 → 评审 → 发布 → 复盘——每个阶段都强制硬契约（产物落到 `docs/`、每功能状态机、实现前必须有红灯测试、阶段交接前自检、自动修复仅一轮），让 AI 的活产出真实可审计的文件，而不是只活在对话里。
...
# 应该输出： pdlc@pdlc-skills  Version: 1.6.1（或更新）  Status: ✔ enabled
```
