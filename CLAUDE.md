...
pdlc-skills is a **Claude Code plugin**. It exposes 37 standardized "Product Development Life Cycle" stages as slash commands (`/pdlc-feature`, `/pdlc-prd`, `/pdlc-tdd`, ..., `/pdlc-onboard`) covering PRD → Design → TDD → Implement → Review → Ship → Deploy → Retro and 23 specialized tools.
...
├── skills/                         ← 37 sub-skills (each = one slash command)
...
1. Visual distinctiveness — typing `/pdlc-` filters cleanly to all 37 PDLC commands; suffix-only names (`/feature`, `/fix`) collide with built-in commands and other plugins.
...
Sub-skills are grouped by `layer:` in frontmatter (the 37 names below all carry the `pdlc-` prefix):
