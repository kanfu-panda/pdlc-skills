...
> **哪些能力仅 Claude Code**：状态栏（§4.5，本就是 shell 脚本非 skill）、自主收敛引擎 `pdlc-loop-run`、配置命令 `pdlc-settings`——这 2 个 skill 不投影到其它平台（`pdlc-loop-next` 逻辑平台中立，已投影）。其它平台靠手动逐阶段驱动达到同样产物。
>
