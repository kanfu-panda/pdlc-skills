...
**不使用 `④/6` 这类「第几/共几」数字**——因为不是每个 feature 都走满 6 段（bug 修复更短、
loop 只覆盖 `tdd→impl→review`），`/6` 会误导。改用**迷你进度条**，天然只铺该 feature 实际经过/将经过的段，高亮当前段：
...
- `PRD·设计·TDD·[实现]·评审·发布`：进度条，`[ ]` 标当前段
- `→评审`：`next_step`，一眼知道该跑什么
- `👤`：运行图标（👤 手动 / 🤖 autonomous / ⛔ blocked / ✅ review_done）
...
autonomous / loop 模式的典型行：

```
● PDLC auth · TDD·[实现]·评审 · →评审 · 🤖 ✓unit ✓lint ✓cov · ⏱3m
```

（进度条此时只铺 loop 覆盖的 `TDD·实现·评审` 三段。）
...
可被项目级 `docs/.pdlc-state/statusline.json` 覆盖）。不用 TOML——状态栏每帧渲染、要 <10ms，
bash 原生解析不了 TOML（得拉 python/toml、每帧多起进程）；JSON 用 jq 读，且**解析一次缓存**，绝不每帧解。
另附一份带注释的 `.example` 供兜底手改。
...
- **渲染 <10ms**：纯 bash + jq。**jq 是本片段引入的软依赖**（并非 pdlc-skills 既有声明依赖）——
  片段设计为 **jq 缺失即静默降级/吐空**（见下），因此不给用户强加安装负担。
- **非 PDLC 项目快速短路**：先判 `docs/.pdlc-state/` 是否存在，不存在**立即吐空退出**
  （绝大多数刷新走这条快路径）。
- **多 feature 懒解析**：按 mtime 排序、只解析最近少数几个（见 §5.4），避免 N 大时超预算。
...
1. impl 阶段 · interactive（默认隐检查、隐 elapsed）
2. loop 收敛中 · autonomous（显检查、显 elapsed、进度条只铺三段）
