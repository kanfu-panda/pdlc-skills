...
| **Layer** | Skill tier. Layer 1 (3) = entry points (`feature`/`fix`/`status`), Layer 2 (11) = core pipeline (PRD/design/tdd/implement/review/ship/...), Layer 3 (24) = specialized tools (arch, standards, relate, test-setup, quality gate, loop tooling, settings, ...). |
...
| **Loop tooling** | `pdlc-loop-next` (prints the next mechanical-convergence command) and `pdlc-loop-run` (convergence engine auto-advancing `tdd → implement → review` until reviewed and awaiting release — `next_step: pdlc-ship`, which loop docs call "`review_done`" — or blocked). Release (`ship`/`deploy`) always stays human. Added v1.2. |
| **Multi-feature loop driver** | `bin/pdlc-loop.sh` (v1.7): drives several features through the same convergence from outside the model, one fresh `claude -p` / `codex exec` process per step; `--parallel N` uses one git worktree per feature, `depends_on` orders them, `--status` (and `/pdlc-status --loop`) reads its run record. Never commits, merges, or ships. [ADR 0006](decisions/0006-multi-feature-loop-driver.md). |
...
| **Platform-neutral core / Tier 1** | The tool-agnostic PDLC methodology (Iron Law + state-machine contract + doc layout + objective checks), distilled in `docs/pdlc-methodology.md` so any AI coding agent (Codex, Cursor, Windsurf, Copilot, Cline, …) can drive PDLC via natural language. Claude Code has the richest integration (Tier 2: 38 slash commands + statusline + autonomous loop). Multi-platform strategy: [ADR 0003](decisions/0003-multi-platform-adapters.md), revised by [ADR 0007](decisions/0007-agent-skills-standard.md). |
| **Agent Skills projection** | Build-time copy of `skills/` that follows the Agent Skills open standard (agentskills.io), produced by `adapters/build_agent_skills.py` and installed with `install.sh --target agents` (or `codex`). How tools other than Claude Code load PDLC; `skills/` itself stays in Claude Code's shape. [ADR 0007](decisions/0007-agent-skills-standard.md). |
| **State-integrity gate** | The admission test a platform must pass before we recommend it for autonomous loops: the `honest-checks` eval plants an un-fixable red test and checks that `last_phase_result.checks` come from real exit codes and that a failed stage does not advance. Codex has passed; Copilot CLI has not. |
